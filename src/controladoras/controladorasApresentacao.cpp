...
std::string formatarCPF(const std::string &entrada)
{
    ...
    {
        throw std::invalid_argument("CPF deve conter exatamente 11 dígitos numéricos (você digitou " +
                                    std::to_string(apenasNumeros.length()) + " dígitos)");
    }
    ...
    while (!cpfValido || !nomeValido || !senhaValida)
    {
        ...
        else if (!senhaValida)
        {
            try
            {
                ...
                std::cout << "(Digite '0' para cancelar, 'r' para reescrever CPF, 'n' para reescrever nome)"
                          << std::endl;
            ...
            catch (const std::invalid_argument &exp)
            {
                ...
                std::cout << "Dica: 6 caracteres com 1 maiuscula, 1 minuscula, 1 numero e 1 simbolo (#$%&)"
                          << std::endl;
            }
        }
    }
    ...
    while (true)
    {
        ...
        case 2: {
            ...
            if (cntrServicoInvestimento->listarCarteiras(cpf, &carteiras))
            {
                ...
                {
                    std::cout << "Código: " << carteira.getCodigo().getValor() << " - " << carteira.getNome().getValor()
                              << std::endl;
                }
 ...
 */
InterfaceManager::InterfaceManager(ControladoraApresentacaoAutenticacao *cntrApresentacaoAutenticacao,
                                   ControladoraApresentacaoUsuario *cntrApresentacaoUsuario,
                                   ControladoraApresentacaoInvestimento *cntrApresentacaoInvestimento)
    : cntrApresentacaoAutenticacao(cntrApresentacaoAutenticacao), cntrApresentacaoUsuario(cntrApresentacaoUsuario),
      cntrApresentacaoInvestimento(cntrApresentacaoInvestimento), telaAtual(TelaAtual::MENU_INICIAL),
