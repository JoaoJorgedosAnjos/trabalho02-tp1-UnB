 ...
 */
CarteiraController::CarteiraController(IServicoInvestimento *servico) : servicoInvestimento(servico)
{
}
        ...
        try
        {
            ...
            if (servicoInvestimento->consultarCarteira(codigo, &carteiraDetalhada, &saldo))
            {
                ...
                if (servicoInvestimento->listarOrdens(codigo, &ordensCarteira) && !ordensCarteira.empty())
                {
                    ...
                    std::cout << std::left << std::setw(8) << "Codigo" << std::setw(12) << "Data" << std::setw(15)
                              << "Valor" << std::setw(10) << "Quantidade" << std::endl;
                    std::cout << std::string(45, '-') << std::endl;

                    for (const Ordem &ordem : ordensCarteira)
                    {
                        std::cout << std::left << std::setw(8) << ordem.getCodigo().getValor() << std::setw(12)
                                  << ordem.getData().getValor() << std::setw(15)
                                  << ("R$ " + ordem.getDinheiro().getValor()) << std::setw(10)
                                  << ordem.getQuantidade().getValor() << std::endl;
                    }
                    std::cout << std::string(45, '-') << std::endl;
...
void CarteiraController::exibirListaCarteiras(const std::list<Carteira> &carteiras)
{
    ...
    std::cout << std::left << std::setw(8) << "Codigo" << std::setw(25) << "Nome" << std::setw(12) << "Perfil"
              << std::endl;
    std::cout << std::string(45, '-') << std::endl;

    for (const Carteira &carteira : carteiras)
    {
        std::cout << std::left << std::setw(8) << carteira.getCodigo().getValor() << std::setw(25)
                  << carteira.getNome().getValor() << std::setw(12) << carteira.getTipoPerfil().getValor() << std::endl;
    }
    std::cout << std::string(45, '-') << std::endl;
...
void CarteiraController::editarCarteiraEspecifica(const Ncpf &cpf, const Carteira &carteiraAtual)
{
        ...
        try
        {
            ...
            if (alterarPerfil == 's' || alterarPerfil == 'S')
            {
                ...
                case 3:
                    ...
                    break;
                default:
                    std::cout << "Opção invalida! Mantendo perfil atual." << std::endl;
                    break;
                }
    ...
    std::cout << "\n=== RESUMO ===" << std::endl;
    std::cout << "Nome : " << carteiraAtual.getNome().getValor() << " -> " << carteiraEditada.getNome().getValor()
              << std::endl;
    std::cout << "Perfil: " << carteiraAtual.getTipoPerfil().getValor() << " -> "
              << carteiraEditada.getTipoPerfil().getValor() << std::endl;
...
bool CarteiraController::excluirCarteiraEspecifica(const Ncpf &cpf, const Carteira &carteiraAtual)
{
    ...
    if (servicoInvestimento->excluirCarteira(carteiraAtual.getCodigo()))
    {
        ...
        std::cout << "A carteira '" << carteiraAtual.getNome().getValor() << "' foi removida permanentemente."
                  << std::endl;
        std::cout << "\nPressione qualquer tecla para continuar..." << std::endl;
        std::cin.ignore();
        std::cin.get();
